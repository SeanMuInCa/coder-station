...
const Comment = (props) => {
	...
	let list = props.commentList.data?.map((item) => (
		<CommentCard key={item._id} commentInfo={item} />
	));
	let showList = list?.slice(
				...
				{user.isLogin ? (
					<>
						<Editor
							...
							ref={editorRef}
						/>
						<Button type="primary" className="mt-5" onClick={handleClick}>
							Submit Comment
						</Button>
					</>
				) : (
